...
- _How much annotated data do I need?_ Unfortunately, there is no simple answer. It depends on complexity of task, the clarity of the information, etc. So as we'll discuss, best practice is to proceed iteratively.

### Annotation challenges

By now it should be clear that your goal in the data annotation phase is to _quickly_ and _correctly_ annotate a _large enough corpus of inputs_ that collectively provide an _adequate representation_ of information you want the model to learn.
...
- Ensure annotators understand the domain and use cases.
- Labeling Instructions
- Guidelines to standardize annotations.
:::

::: {.callout-note collapse="true" icon="false"
     title="Have a quality control process"}
At this basic level, you should conduct reviews to ensure compliance with instructions.

Regular checks, cross-validation, and feedback loops.
  * Consensus mechanisms: have more than one person annotate each item, and measure consistency
...
- **Inter-annotator agreement (IAA)** is a _consensus_ metric that measures the degree to which different annotators provide similar annotations.
  - Cohen's kappa (to compare 2 labelers)
  - Fleiss' kappa (to compare >2 labelers)
  - Krippendorf's alpha
- F1 score
:::
...
Start small, refine, and scale gradually.
:::
...
**Instance Segmentation** separately detects and segments each object instance. It's therefore similar to semantic segmentation, but _identifies the existence, location, shape, and count of objects_. It is best for distinct and countable "things". Notice the separately identified four bears in the image below:
...
- __Model-suggested labels__: A pre-trained model provides hints, e.g. multiple options of likely object classes, and the human makes the decision.
...
- __Active Learning__: A model is learning how to annotate the images alongside the human, and actively decides which images the human should label to accelarate model training the fastest.
:::
...
This step can include various activities. Here are some illustrative tasks:
...
**Remember this key best practice**: Iterate! Annotate, train, test, fix annotations, figure out whether/how to do more and/or better annotations, refine your annotation approaches, etc.
...
We've discussed the importance of having high quality annotations. Some tools leave it entirely up to you to figure out how to measure this, but others have assessment capabilities built into the tool.

Firstly this may include calculation of various annotation quality metrics:

This may include features such as:
- Providing configurable mechanisms for
