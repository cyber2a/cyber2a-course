...
Since large-scale datasets for segmentation are unavailable, in developing SAM, the research team used a data engine to
generate segmentation masks, which were then manually annotated. The model was trained on diverse images to improve its
generalization capabilities. This model-in-the-loop self-supervised training created two datasets: SA-1B containing
1 billion segmentation masks from about 11M privacy preserving images and SA-V dataset containing 642.6K masklets
